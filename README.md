...
## Сначала настроить время задание 11
...
nameserver 192.168.100.62
```
Не удаляй то, что уже настроено. Просто добавь в конец файла строку nameserver 192.168.100.62.
Но есть нюанс: если ты уже фиксировал resolv.conf через chattr +i, его нужно сначала разблокировать, потом отредактировать, потом снова заблокировать.
Как правильно отредактировать /etc/resolv.conf
- Шаг 1. Разблокировать файл (если блокировал)
```
chattr -i /etc/resolv.conf
```
...
nano /etc/resolv.conf
```
- Шаг 3. Привести файл к такому виду
Должно быть так (и ничего лишнего):
```
text
search au-team.irpo
nameserver 192.168.100.62
...
search au-team.irpo — чтобы можно было писать hq-srv, а не полное имя hq-srv.au-team.irpo

nameserver 192.168.100.62 — твой локальный DNS-сервер (главный)

nameserver 1.1.1.1 и 8.8.8.8 — запасные (если локальный не ответит)

Шаг 4. Сохранить и закрыть
Ctrl+O, Enter, Ctrl+X

Шаг 5. Снова заблокировать файл (чтобы DHCP не перезаписал)
```
chattr +i /etc/resolv.conf
```
</details>
